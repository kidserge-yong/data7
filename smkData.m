classdef smkData
    ...
    methods
        ...
        function EMG_array = translate2EMG(obj, str_array)
            EMG_array = [];
            for n = 1:length(str_array)
                obj = obj.loadData(116, [int32(char(str_array(n))), 0,0,0]);
                if length(obj.emg) == 32
                    EMG_array = [EMG_array; obj.emg];
                end
            end
        end
        
        function obj = loadData(obj, start_byte, data_byte)
        ...
        function UARTdata = getUART(obj)
            UARTdata = [];
            if obj.serial.NumBytesAvailable > 0
                UARTdata = read(obj.serial, obj.serial.NumBytesAvailable, "uint8");
            end
        end
